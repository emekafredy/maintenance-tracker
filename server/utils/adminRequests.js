...
class AdminRequests {
  ...
  static processRequestQuery(request, response, proccesdAt, processedField) {
    ...
    const query = {
      ...
      values: [
        request.body.requestStatus.toLowerCase(),
      ...
      client.query('SELECT * FROM requests WHERE requestId = $1', [reqId])
        .then((data) => {
          response.status(200).json({
            message: `Request has been ${processedField}`,
            data: data.rows,
          });
  ...
  static selectionQuery(request, response, proccesdAt, processedField) {
    const reqId = parseInt(request.params.requestId, 10);
    ...
    return client.query('SELECT * FROM requests WHERE requestId =$1', [reqId])
      .then((data) => {
        ...
        return AdminRequests.processRequestQuery(request, response, proccesdAt, processedField);
      }).catch(error => response.status(404).json({ message: error.message }));
  }

  static resolveSelectionQuery(request, response, proccesdAt, processedField) {
    const reqId = parseInt(request.params.requestId, 10);
    client.query('SELECT * FROM requests WHERE requestId =$1', [reqId])
      .then((data) => {
        if (data.rows.length === 0) {
